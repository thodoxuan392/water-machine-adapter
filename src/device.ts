...
export class Device implements DeviceInterface {
    ...
    this._timerToCheckDevicePort = setInterval(async () => {
      ...
      this._port.on('readable', () => {
        ...
        while (this._buffer.length > 0) {
          const { success, cutLen } = this._handleReadBufferFromDevice(
            this._buffer,
          );
          if (!success) {
            ...
            this._buffer = this._buffer.slice(cutLen, this._buffer.length);
          }
        }
      });
  ...
  public async sendCommandPlayAudio(command: CommandPlayAudio): Promise<void> {
    const data = [];
    data.push(START_BYTE);
    data.push(command.protocolId);
    data.push(2);
  ...
  public async sendCommandUpdateRFID(
    ...
    data.push(Number(command.isValid));
    data.push((command.volume >> 24) & 0xff);
    data.push((command.volume >> 16) & 0xff);
    data.push((command.volume >> 8) & 0xff);
    data.push(command.volume & 0xff);
  ...
  private _handleReadBufferFromDevice(buffer: Buffer): {
    ...
    switch (protocolId) {
      ...
      case ProtocolId.RFID_DETECTED: {
        ...
        const isValid = Boolean(buffer.at(5 + rfidLen));
        const volume =
          (buffer.at(6 + rfidLen) << 24) |
          (buffer.at(7 + rfidLen) << 16) |
          (buffer.at(8 + rfidLen) << 8) |
          buffer.at(9 + rfidLen);
        const issueDate = [
